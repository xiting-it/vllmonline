...
启动 vllmonline 后浏览器打开 **`http://<host>:8080/portal`**（原生 HTML/CSS/JS 单页，Chart.js 本地内嵌，零外部依赖，离线可用）。

| 面板 | 内容 |
|------|------|
| Hero | vllmonline / vLLM v1 / vLLM v2 在线状态徽章（2 秒轮询） |
| 模型管理 | 模型卡片墙 + Load / Sleep / Wake / Unload 按钮 |
| 灰度控制台 | 流量配比环形图、阶段进度条、启动灰度 / 推进 / 回滚 / 发 20 个测试请求按钮 |
| 实时监控 | v1 vs v2 累计请求曲线（解析 `/metrics`） |
| A/B 评测 | 样本量 / p 值 / Cohen's d 统计卡片、三维度条形图、结论解读 |

**实测快照回填**：A/B 评测和部署历史在数据库为空时，自动展示 MI300X 上真实运行的结果
（n=49、p=0.0001、d=-0.83、`canary-9543394eec75`），面板顶部有「⬗ MI300X 实测快照」标签；
一旦数据库里有真实数据（如重跑 `run_ab_eval.py` 或启动新灰度），自动切换为实时数据。

演示动线：打开门户 → 点「启动灰度」（饼图 90:10）→ 点「发送测试请求」（监控曲线 v2 计数）
→ 点「推进」（饼图 70:30）→ A/B 面板看统计结论 → 点「回滚到 v1」（饼图瞬间 100:0，v2 卡片变 SLEEPING）。
